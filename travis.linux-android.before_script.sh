...
cd ..
rust/configure --target=arm-linux-androideabi --android-cross-path=`pwd`/ndk-standalone --disable-docs --disable-optimize-tests --llvm-root=`pwd`/local-llvm --enable-fast-make --enable-clang
